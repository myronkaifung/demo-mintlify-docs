...
<Note> Avoid using names containing `$` or `mp` (e.g. `$name`, `mp_country_code`, etc.) as they are used by our [reserved event properties](/docs/data-structure/property-reference/reserved-properties#reserved-event-properties) which receive special treatment in our UI or are used for special processing. </Note>
...
<Note>UTM tracking is case sensitive and should be formatted in lowercase as shown in the examples above.</Note>
...
<Warning>We recommend against calling `.identify()` for anonymous visitors to your site.</Warning>
